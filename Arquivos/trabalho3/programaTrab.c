...
int main(void){
	...
	char arquivoINDX[30];			//nome do arquivo de indx
	...
	if(opcao == 8){ //le o arquivo de registro e cria um arquivo indx 
		
		//leitura do nome dos arquivos
		scanf(" %[^ \n]", arquivoBIN);
		scanf(" %[^ \n]", arquivoINDX);

		if(indx_cria(arquivoBIN,arquivoINDX)){

			binarioNaTela(arquivoINDX);//funcao para imprimir binario
	
		}

	}

	if(opcao == 9){ //imprime o Registro do RRN dado, caso exista registro

		//leitura do nome do arquivo
		scanf(" %[^ \n]", arquivoBIN);
		scanf(" %[^ \n]", arquivoINDX);

		indx_search(arquivoBIN,arquivoINDX);
	}

	if(opcao == 10){ //insere no fim do arquivo de registro e na arvore do arquivo de indx
		
		//leitura do nome dos arquivos
		scanf(" %[^ \n]", arquivoBIN);
		scanf(" %[^ \n]", arquivoINDX);

		if(registro_indx_inserir(arquivoBIN,arquivoINDX)){
